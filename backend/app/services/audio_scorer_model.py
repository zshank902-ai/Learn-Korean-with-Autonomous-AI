...
def build_audio_scorer_model(input_shape=(128, 128, 1)):
    ...
    x = layers.Conv2D(
        32, (3, 3), activation="relu", padding="same", name="Audio_Conv1"
    )(input_audio)
    ...
    x = layers.Conv2D(
        64, (3, 3), activation="relu", padding="same", name="Audio_Conv2"
    )(x)
    x = layers.BatchNormalization()(x)
    x = layers.MaxPooling2D((2, 2))(x)
    ...
    x = layers.Conv2D(
        128, (3, 3), activation="relu", padding="same", name="Audio_Conv3"
    )(x)
    x = layers.BatchNormalization()(x)
    ...
    embedding = layers.Dense(256, activation="linear",
                             name="Acoustic_Embedding")(x)

    # 5. Final Model
    model = models.Model(inputs=input_audio,
                         outputs=embedding, name="audio_scorer_v1")
