...
def build_ocr_model(img_width=256, img_height=64, max_label_len=32, num_chars=1200):
    ...
    # Block 1
    x = layers.Conv2D(32, (3, 3), activation="relu", padding="same", name="Conv1")(
        input_img
    )
    ...
    x = layers.MaxPooling2D((1, 2), name="pool3")(
        x)  # Selective downsampling for width
    ...
    output = layers.Dense(
        num_chars + 1, activation="softmax", name="dense2")(x)
