...
class SentenceCorrector:
    ...
    async def async_get_correction(self, sentence: str) -> dict:
        ...
        if GROQ_API_KEY:
            try:
                return await loop.run_in_executor(None, partial(self._call_groq_sync, sentence))
            except Exception as e:
                print(f"Corrector: Groq API failed ({e}), falling back to local Ollama...")
        else:
            print("Corrector: GROQ_API_KEY missing, using local Ollama (Phi-3).")

        # 2. Try Local Ollama (Phi-3)
        try:
            return await loop.run_in_executor(None, partial(self._call_ollama_sync, sentence))
