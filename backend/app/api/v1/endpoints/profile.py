...
from app.schemas.profile import ProfileUpdate, NicknameCheckResponse, PasswordChangeRequest, PasswordChangeResponse
...
def update_profile(
    ...
    if update_data.nickname is not None and update_data.nickname != current_user.nickname:
        existing = db.query(User).filter(User.nickname == update_data.nickname).first()
        if existing:
...
def check_nickname(
    nickname: str,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user) # Require auth as requested
):
    if not nickname:
