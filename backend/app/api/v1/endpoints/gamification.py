...
async def add_xp(request: XPRequest, current_user: User = Depends(get_current_user)):
    ...
    """
    new_xp = gamification_manager.update_stat(current_user.id, "xp", request.amount)
    ...
    if new_level > current_level:
        gamification_manager.redis.hset(gamification_manager._get_user_key(current_user.id), "level", new_level)
        leveled_up = True
...
def user_level_up(request: LevelUpRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """
...
async def complete_quest(quest_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """
    Optimized quest completion with buffered XP updates.
    """
    gamification_manager.update_stat(user_id=current_user.id, field="xp", amount=100)
...
def get_daily_quests(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    quests = db.query(DailyQuest).filter(DailyQuest.user_id == current_user.id).all()
    
    if not quests:
        seeded_quests = [
            DailyQuest(title="Scan 3 Street Signs", xp_reward=50, is_completed=False, user_id=current_user.id),
            DailyQuest(title="Speak for 5 Minutes", xp_reward=100, is_completed=False, user_id=current_user.id),
            DailyQuest(title="Listen to Gukje Market Dialogue", xp_reward=30, is_completed=True, user_id=current_user.id),
        ]
