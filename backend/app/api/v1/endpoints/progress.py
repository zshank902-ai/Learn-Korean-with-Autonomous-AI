...
from app.models.user import User, TopikProgress, TopikStatus
    ...
    if level > 1:
        ...
        if not is_prev_complete:
            ...
            return {
                "level": level,
                "locked": True,
                "modules": [{"module_id": m, "status": "locked"} for m in ["mod1", "mod2", "mod3"]]
            }

    # Fetch existing progress
    progress_records = db.query(TopikProgress).filter(
...
def complete_module(
    ...
    progress = db.query(TopikProgress).filter(
        TopikProgress.user_id == current_user.id,
    ...
    try:
        ...
        if current_idx + 1 < len(sequence):
            ...
            next_prog = db.query(TopikProgress).filter(
                TopikProgress.user_id == current_user.id,
                TopikProgress.topik_level == data.topik_level,
