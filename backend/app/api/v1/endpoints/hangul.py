...
def lookup_word(word: str = Query(..., min_length=1, max_length=50)):
    ...
    if not GROQ_API_KEY:
        # Fallback if no LLM
        return LookupResponse(word=word, romanization="", meaning="LLM Offline", example="", difficulty=0)
    ...
    try:
        ...
        return LookupResponse(word=word, romanization="", meaning="Dictionary lookup failed", example="", difficulty=0)
...
def update_progress(body: ProgressUpdate, db: Session = Depends(get_db)):
    ...
    if not prog:
        ...
        total_acc = (prog.average_accuracy * prog.quizzes_taken) + body.accuracy
