...
DATASET_DIR = os.path.join(
    os.path.dirname(__file__), "..", "..", "..", "datasets", "ocr"
)
...
def download_korean_font():
    ...
    if not os.path.exists(font_path):
        ...
        try:
            ...
            print(
                f"Failed to download font: {e}. Falling back to default system font (might cause tofu boxes)."
            )
            return None
    return font_path


def apply_augmentations(img: Any) -> Any:
    """Applies realistic noise and slight rotations to mimic real-world signage."""
    # 1. Slight Rotation (-2 to 2 degrees)
    angle = random.uniform(-2, 2)
    img = img.rotate(angle, resample=2, fillcolor="white")
