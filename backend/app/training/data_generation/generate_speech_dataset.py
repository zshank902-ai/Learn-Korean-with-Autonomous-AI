...
DATASET_DIR = os.path.join(
    os.path.dirname(__file__), "..", "..", "..", "datasets", "audio"
)
...
def generate_synthetic_mfcc(is_native: bool) -> np.ndarray:
    ...
    """
    base_mfcc = np.sin(np.linspace(0, 10, MAX_AUDIO_LENGTH))[:, np.newaxis] * np.ones(
        (1, MFCC_BANDS)
    )
...
def generate_audio_dataset(num_samples=100):
    print(
        f"Generating Synthetic Audio (MFCC) Dataset with {num_samples} samples...")
    ensure_directories()
    ...
    for i in range(num_samples):
        ...
        fluency_score = (
            np.random.randint(
                90, 101) if is_native else np.random.randint(40, 85)
        )
